...
// If additional arguments exist, prefer std::format where available.
// Note: Android NDK/libc++ configurations may provide <format> but not the full
// C++20 formatting library (e.g. std::format_string). For those toolchains we
// fall back to returning the raw format string so the build succeeds.
template <typename... Args>
inline std::string format_or_pass(const char* fmt_str, Args&&... args) {
